...
对原驱动仅请求解码选定的九帧，并检查读后帧位置、25fps 和原始 1024 RGB 尺寸。OpenCV seek 的底层编解码器可能内部解码 GOP 依赖帧；脚本不会将那些依赖帧载入结果或送进模型。原帧未经缩放，用 FFmpeg FFV1/BGR0 写入内部 AVI，再逐帧重新解码，要求九帧 RGB 与原解码数组**完全相等**，且无多余帧。视频有损原文件本身的解码 RGB 是本次等值依据，不声称恢复压缩前相机数据。
